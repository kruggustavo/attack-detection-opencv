...
\newpage
\section*{Abstract}
The purpose of the following thesis work is to develop a computer application capable of detecting certain criminal activity expressed through human poses captured by video cameras in real time. \par

The idea arises from the observation of numerous criminal acts in which the attack or aggression with weapons were decisive in reducing and neutralizing the victims thus delaying the police response. \par

It is intended to provide an application solution that serves as a tool for detecting possible cases of human threat and aggression in order to issue an early warning in case the victim is unable to ask for help quickly and by their own means.\par 

Thus, for the development of the solution, artificial intelligence concepts were put into practice and tools that comply with the state of the art were used to detect criminal human activity.\par 

Keywords: \ textit {Artificial Intelligence, Neural Networks, Poses and Human Activities, Video, Training, Detection}.
...
\section[Planteamientos]{Planteamiento del problema}
...
\begin{enumerate}
	\baselineskip 16pt
	\item Entrenar redes neuronales mediante secuencias de poses humanas establecidas
para el fin.\par 
	\item Analizar imágenes secuenciales captadas por cámaras para video.
\par 
...
A lo largo del tiempo, los sistemas de vigilancia han mejorado la tecnología utilizada en los dispositivos de monitoreo, sensores, etc. \par
 
Según M. Valera y S.A. Velastin. \cite{valera}, pueden diferenciarse hasta el momento tres generaciones de sistemas de vigilancia:
\begin{enumerate}
	\baselineskip 16pt
...
Dentro de la clasificación de sistemas de visión para vigilancia, R. Dautov et al \cite{dautov} cita cuatro tipos distintos:
\begin{itemize}
...
En el artículo de O.C.Ann y L.B.Theng \cite{activity1} se citan algunas estructuras de detección de actividad humana en donde se utilizan:
\begin{itemize}
...
Según Doménech \cite{human1} existen por lo menos tres categorías de métodos de representación de poses humanas de acuerdo a como se interpreta la estructura del cuerpo:
...
\section[Machine Learning]{Conceptos de Machine Learning}\label{machinelearning}
...
\section[Clasificación]{Métodos de clasificación}\label{clasificacion}
...
S. Bailey et al \cite{bailey} describe algunos métodos principales de clasificadores, también se puede agregar los citados por N. S. Kamarudin et al\cite{kamarudin}, y los métodos de regresión de Andrew Ng y su curso de Machine Learning  \cite{machinelearningcoursera}. \par
...
De Matich \cite{matich} se pueden entresacar los tipos de funciones de entrada más comunes:
\begin{enumerate}
	\baselineskip 16pt
...
De Enyinna et al\cite{enyinna} se pueden citar tres de los tipos de funciones de activación más conocidos:
...
Algunos de los optimizadores más conocidos:
\begin{itemize}
	\baselineskip 10pt
...
\newpage
\chapter[Procesamiento gŕafico]{Procesamiento gráfico}
\section[Visión por Comp.]{Visión por computadora}\label{visionporcomputadora}
...
Normalmente, el proceso de identificación de objetos en imágenes se da en diferentes etapas descritas en \cite{lozano} y \cite{kamarudin}, ellas son:	
...
\subsubsection{a) Detección de objetos en imágenes}
...
Enfocado a la seguridad, la detección de objetos trata de identificar solamente unas pocas categorías de objetos en imágenes, lo que lo hace un proceso más viable: la detección de formas humanas, también denominado detección de peatones (Pedestrian detection \cite{zhang1}).

\subsubsection{Técnicas de detección de objetos en imágenes}
A través de los años, como consecuencia de las investigaciones en visión por computadora, han surgido	diferentes enfoques en la utilización de técnicas de detección de objetos, M. Valera y S.A. Velastin \cite{valera} citan dos enfoques a lo que se pueden agregar además el presentado en \cite{nidhi} y el citado en \cite{barron}:
\begin{enumerate}
	\baselineskip 16pt
...
\subsubsection{b) Clasificación de objetos}
...
\subsubsection{c) Extracción de características de objetos}
...
\subsubsection{d) Análisis del comportamiento de los objetos}
...
\begin{figure}[h!]
	...
	\centering
	\caption{imagen de un hombre, y los gradientes obtenidos de la misma. Fuente Dalal y Trigg \cite{hog1} }
	\label{fig:hog1}
\end{figure}
 

\section[Herramientas]{Herramientas de visión por computadora}\label{herramientasdevisionporcomputadora}
...
En las redes convolucionales, se aplican ciertos filtros a las imágenes mediante neuronas convolucionales resumiendo así ciertas porciones de la imagen y obteniendo una representación más detallada de la misma, este proceso de resumir porciones se denomina reducción de muestreo, y en inglés se lo conoce como \textit{pooling} (pooling layer, es la capa encargada de resumir porciones de la imágen). \par
...
\section[Pruebas]{Pruebas de estimación de poses humanas}\label{pruebaspose}
...
Cada configuración de pose distinta, con sus imágenes y anotaciones es almacenada en un directorio diferente a fin de poder simular la representación de un objeto con sus anotaciones.\par 
...
El entrenamiento de clasificadores SVM, como ya se explicó en un capítulo anterior, consiste en introducir valores al clasificador para establecer un límite de decisión en los datos de salida, de manera que pueda responder a un nuevo valor clasificándolo según si corresponde o no, a las características de los valores de entrenamiento. En las SVM, el límite de decisión es llamado un hiperplano.\par
...
\subsubsection{Descripción}
\textit{Convolutional Architecture for Fast Feature Embedding} (Caffe) en inglés, o Arquitectura convolucional para incrustación rápida de características, es un framework para desarrollo de software enfocado a Machine Learning, desarrollado en la Universidad de Berkeley en California, Jia et all explican su desarrollo en \cite{caffe}.\par 
...
Un modelo de red neuronal consiste en un conjunto de configuraciones para una red previamente entrenada por un desarrollador, con sus pesos, capas intermedias y las funciones de entrada, activación y salida correspondientes.\par 
...
En la documentación oficial de OpenCV se puede encontrar ejemplos de cómo utilizar estos modelos pre entrenados, que, siendo implementados en unas lineas de código Python, permitieron poder utilizar el modelo para estimar poses humanas en imágenes (un video es un conjunto de imágenes).
...
\subsubsection{Conclusión}
Tomando en cuenta que la velocidad de procesamiento utilizando Caffe es ligeramente superior a las pruebas anteriores, se optó por utilizar el modelo para la extracción de poses. Una imagen procesada en un computador con dos núcleos tomó en promedio 8 segundos, sin GPUs para efectuar multiprocesamiento.\par
...
\section[Poses]{Extracción de poses humanas}\label{extraccionpose}
...
\section[Ángulos]{Cálculo angular de extremidades}\label{calculoangular}
...
\section[RNA]{Entrenamiento de una red neuronal artificial}\label{entrenamiento}
...
La conclusión: se decidió por un modelo sencillo de 3 capas, detallado en la Prueba 6. No se realizaron más pruebas.

\section[Predicción]{Predicción de poses compatibles con amenazas o agresiones}\label{prediccion}
En este punto, la red entrenada ya está lista para inferir. Para el efecto, se escribió una aplicación en Python que pueda cargar el modelo pre entrenado de poses y, a medida que vaya captando imágenes de la cámara incorporada, pueda determinar si existe o no una pose compatible con agresión en la imagen.\par
...
\section[Armas]{Detección de armas utilizando detectores en cascada}\label{deteccionarmas}
...
A través del trabajo desarrollado, se evidencia el cumplimiento del objetivo general que consistía en desarrollar una solución computacional que permita la detección básica de delitos de agresión o amenazas humanas, mediante el entrenamiento de redes neuronales de computadoras.\par

Además, se cumplieron los objetivos específicos que componen el objetivo general y se detallan a continuación:

En la sección \ref{posturaypose} primero se estudia la pose humana, estableciendo que la actividad que realiza una persona puede ser expresada por la disposición de sus extremidades.\par

En las secciones \ref{machinelearning} y \ref{clasificacion} se detallan conceptos y técnicas de Machine Learning, como redes neuronales, que se utilizaron para diferenciar distintas poses en una persona observada.\par

En las secciones \ref{visionporcomputadora} y \ref{herramientasdevisionporcomputadora} se explican los principios de visión por computadora y se citan las diferentes herramientas con que se cuentan actualmente.\par

En la sección \ref{pruebaspose} se enumeran las distintas pruebas realizadas con herramientas de detección de poses disponibles en la red, a fin de seleccionar aquella herramienta óptima para ser ejecutada en el entorno.\par

En la sección \ref{extraccionpose} se explican las etapas del proceso de extracción de poses humanas durante el desarrollo de la aplicación, como también la forma en que éstas se representan.\par

En la sección \ref{calculoangular} se explica la idea detrás del cálculo angular que se efectúa entre las líneas formadas por las extremidades de una pose detectada. El orden y representación también se detallan.\par

En la sección \ref{entrenamiento} se describe el proceso de entrenar una red neuronal con los ángulos de poses que sugieran amenazas o agresiones, y también aquellas poses que no sean compatibles.\par

En la sección \ref{prediccion} se demostró que es posible clasificar con redes neuronales aquellas poses que sugieran agresiones humanas en imágenes de video.\par

En la sección \ref{deteccionarmas} además, se logró la detección de armas de fuego cortas cuando se detectan poses de agresión, complementando así el objetivo de desarrollar una solución aplicativa para la detección de agresiones humanas en imágenes secuenciales.\par


\chapter{Trabajos a futuro}
Una alternativa al presente trabajo consistiría en la utilización de múltiples fuentes (cámaras) para la recolección de imágenes haciendo de la aplicación una herramienta para la detección completa en entornos visuales más amplios.\par

Además, no todas las poses existentes consideradas como agresiones o amenazas fueron establecidas para el entrenamiento, ya que la cantidad de posibilidades es muy amplia. Se podría también, incluir una mayor cantidad de tipos de armas que puedan ser utilizadas por el humano para proferir agresiones.\par

Otra posibilidad sería la de implementar un detector de poses mediante redes neuronales convolucionales más sencillas que no requieran el uso de GPUs. Las opciones son amplias.\par
